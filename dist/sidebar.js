...
function onNewInput(chunk) {
    ...
    if (chunk === "\u001b") {
        ...
        if (!input)
            setMessage("Type a directory path, then press Tab to complete", 2000);
        else {
            const completed = completeDirectoryPath(input, process.cwd(), process.env.HOME);
            input = completed.value;
            if (completed.ambiguous)
                setMessage("Tab completed common prefix", 1500);
            else if (!completed.matched)
                setMessage("No matching directory", 1500);
        }
    }
    else if (chunk === "\u007f")
...
function killSession(session) {
    ...
    try {
        if (isActive && rightPane && liveSessions.length > 0) {
            ...
            if (replacement.tmuxPaneId !== rightPane)
                tmux(["swap-pane", "-s", replacement.tmuxPaneId, "-t", rightPane]);
            tmux(["select-pane", "-t", replacement.tmuxPaneId]);
            if (session.tmuxPaneId && paneExists(session.tmuxPaneId))
...
function repairMissingRightPane(cwd) {
    const panes = tmux(["list-panes", "-t", `${TMUX_SESSION}:workbench`, "-F", "#{pane_id}"]).split("\n").filter(Boolean);
    if (panes.length >= 2)
        return;
    const id = randomUUID();
    ...
    if (leftPane)
        tmux(["resize-pane", "-t", leftPane, "-x", String(SIDEBAR_WIDTH)]);
...
function focusStartedSession(id) {
